...
%if 0%{?rhel} && 0%{?rhel} < 10
# RHEL 9: use pip + our locally packaged poetry-core (avoids python3-build/installer)
...
%else
# RHEL 9: PyYAML available, but sphinx_rtd_theme is not — skip docs build
BuildRequires:  python%{python3_pkgversion}-PyYAML
%endif
...
%if 0%{?rhel} && 0%{?rhel} < 10
# Patch Makefile so man page install loop gracefully handles empty doc/build/man/
# (we skip sphinx docs on RHEL 9). Replace bare glob with shell command substitution
# so an empty directory doesn't cause "cannot stat *.8" failure.
sed -i 's|for man in doc/build/man/\*.8;|for man in $$(ls doc/build/man/*.8 2>/dev/null);|' Makefile
%endif
...
# Build documentation (skip on RHEL 9 — sphinx_rtd_theme not available)
%if ! (0%{?rhel} && 0%{?rhel} < 10)
make -C doc man
%else
# Create empty doc dir so make install doesn't fail on missing man pages
mkdir -p doc/build/man
%endif

# Build application wheel
%if 0%{?rhel} && 0%{?rhel} < 10
# RHEL 9: pip + poetry-core does both build and install in one step (see %%install)
%else
...
# Install application
%if 0%{?rhel} && 0%{?rhel} < 10
# RHEL 9: pip handles both build and install in one command
python3 -m pip install --no-build-isolation --no-deps \
    --prefix=/usr --root=%{buildroot} .
%else
...
%if 0%{?rhel} && 0%{?rhel} < 10
# On RHEL 9, use umoci (packaged separately) which works without SYS_ADMIN capability
# buildah's commit step requires CAP_SYS_ADMIN for namespace operations (remount /)
# umoci creates OCI images via tar archives without needing mount namespaces
cat > %{buildroot}/%{_sysconfdir}/kiwi.yml << 'KIWIEOF'
oci:
  - archive_tool: umoci
KIWIEOF
%endif
