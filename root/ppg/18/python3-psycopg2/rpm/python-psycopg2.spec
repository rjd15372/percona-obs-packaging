...
BuildRequires:  percona-postgresql18-devel
BuildRequires:  gcc
...
%{__ospython} setup.py build_ext --pg-config /usr/pgsql-18/bin/pg_config build
...
%install
export CFLAGS=${RPM_OPT_FLAGS} LDFLAGS=${RPM_LD_FLAGS}
%{__ospython} setup.py build_ext --pg-config /usr/pgsql-18/bin/pg_config install --no-compile --root %{buildroot}
%if !0%{?suse_version}
