# PERCONA REBUILD of the Rocky Linux 8 krb5 source package, for
# ppg:common:deps (RockyLinux_8 repository only).
#
# Why: the EL8-built Percona PostgreSQL binary tarball (ssl1.1 variant)
# bundles krb5-libs. Red Hat builds krb5 with --with-crypto-impl=openssl
# against Red Hat's OpenSSL 1.1.1 fork, so libk5crypto.so.3 needs symbol
# version nodes (EVP_KDF_ctrl@OPENSSL_1_1_1b) that exist ONLY in
# RHEL-family libcrypto.so.1.1 — the bundled library fails to load on
# stock OpenSSL 1.1 hosts (proven on Debian 11). The official Percona
# tarball's krb5 has no libcrypto dependency at all.
#
# Delta vs. the distro spec (kept minimal, marked "PERCONA"):
#   - --with-crypto-impl=builtin (was openssl): krb5's builtin crypto,
#     no libcrypto linkage in any krb5-libs shared object.
#   - --with-tls-impl=no (was openssl): k5tls.so (shipped in krb5-libs)
#     becomes the upstream "notls" stub instead of linking OpenSSL;
#     KKDCP/HTTPS KDC transport degrades gracefully.
#   - --without-spake-openssl: spake.so (shipped in krb5-libs) supports
#     only the native edwards25519 group (the default) instead of also
#     offering the OpenSSL-backed NIST P-256/384/521 groups.
#   - Patch1000 detaches the RH downstream FIPS/KDF hooks from OpenSSL
#     in code shared by all crypto backends (see the patch header).
...
# PERCONA: ".percona.N" appended to the distro Release (34) so that this
# rebuild's EVR beats 1.18.2-34.el8_10 during dependency resolution while
# still yielding to a future distro 1.18.2-35 security rebase.
Release: 34.percona.1%{?dist}
...
# PERCONA: with --with-crypto-impl=builtin the RH downstream FIPS/KDF
# patches would still drag OpenSSL into shared code (configure hard-fails
# without the RH-only EVP_KDF_* backports; prng.c, libkrad and spake.so
# would link libcrypto). This patch confines those hooks to openssl
# builds. Numbered 1000 to stay clear of future distro patch numbers.
Patch1000: percona-builtin-crypto-portability.patch
...
# PERCONA: openssl-devel is still needed for the PKINIT plugin (separate
# krb5-pkinit subpackage, never bundled into the tarball). The libraries
# in krb5-libs are built with the builtin crypto impl and do not link it.
...
# PERCONA: openssl-libs Requires dropped — with the builtin crypto impl
# no krb5-libs shared object links libcrypto/libssl.
...
%dir %{_libdir}/krb5/plugins
%dir %{_libdir}/krb5/plugins/*
# PERCONA: with --with-tls-impl=no this is the upstream "notls" stub
# (no OpenSSL linkage); KKDCP/HTTPS KDC transport reports TLS as
# unsupported instead of dlopen-failing.
...
%changelog
* Wed Jul 22 2026 Percona Development Team <info@percona.com> - 1.18.2-34.percona.1
- Rebuild with --with-crypto-impl=builtin and --with-tls-impl=no so that
  no krb5-libs shared object links OpenSSL. Red Hat's build needs symbol
